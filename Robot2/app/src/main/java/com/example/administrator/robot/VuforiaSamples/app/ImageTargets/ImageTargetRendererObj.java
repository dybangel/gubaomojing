...
package com.example.administrator.robot.VuforiaSamples.app.ImageTargets;
...
import com.example.administrator.robot.SampleApplication.SampleApplicationSession;
import com.example.administrator.robot.SampleApplication.utils.FileUtils;
import com.example.administrator.robot.SampleApplication.utils.LoadingDialogHandler;
import com.example.administrator.robot.SampleApplication.utils.SampleMath;
...
public class ImageTargetRendererObj implements GLSurfaceView.Renderer {
    ...
    private float fovy;
    private Mesh[] mesh;
   // public  String path1 = Environment.getExternalStorageDirectory()+ File.separator ;
    //public String name="book.png";
    //+ "book.png";
    public ImageTargetRendererObj(ImageTargets activity, SampleApplicationSession session) {
        ...
        world.setClippingPlanes(12.0f, 3000.0f);
        ...
        if (!TextureManager.getInstance().containsTexture("texture")) {
            Texture texture = new Texture(BitmapHelper.rescale(BitmapHelper.convert(
                    mActivity.getResources().getDrawable(R.drawable.color)), 512, 512));
         //  Texture texture = new Texture(BitmapHelper.convert(mActivity.getResources().getDrawable(R.drawable.a)))
            TextureManager.getInstance().addTexture("texture", texture);
        }

//        Texture texture = null;
//        try {
//            texture = new Texture(BitmapHelper.rescale(BitmapHelper.loadImage(new FileInputStream(path1)), 256, 256));
//        } catch (Exception e) {
//            e.printStackTrace();
//        }
...
//            TextureManager.getInstance().addTexture(name, texture);
       ...
       // FileUtils.copayAssetsToSdCard(activity.getAssets());
        try {
         //   mActivity.getResources().getAssets().open("ball.obj");
          //  mActivity.getResources().getAssets().open("ball.mtl");

            //  tmp = Loader.loadOBJ(new FileInputStream(FileUtils.path+"zhanzhedenane.obj"),new FileInputStream(FileUtils.path+"zhanzhedenane.mtl"), 2);
         //   tmp = Loader.loadOBJ(new FileInputStream(FileUtils.path+"am.obj"),new FileInputStream(FileUtils.path+"am.mtl"), 2);
            //tmp = Loader.loadOBJ(new FileInputStream(FileUtils.path+"book.obj"),new FileInputStream(FileUtils.path+"book.mtl"), 2);
           // tmp = Loader.loadOBJ(new FileInputStream(FileUtils.path+"banana.obj"),new FileInputStream(FileUtils.path+"book.mtl"), 2);
          //  tmp = Loader.loadOBJ(new FileInputStream(FileUtils.path+"ball.obj"),new FileInputStream(FileUtils.path+"ball.mtl"), 2);
            tmp = Loader.loadOBJ(mActivity.getResources().getAssets().open("111.obj"),mActivity.getResources().getAssets().open("111.mtl"), 2);
...
//                .sendEmptyMessage(LoadingDialogHandler.HIDE_LOADING_DIALOG);
    ...
    private void renderFrame() {
        ...
        for (int tIdx = 0; tIdx < state.getNumTrackableResults(); tIdx++) {
          // for (int tIdx = 0; tIdx < ; tIdx++) {
            // get the trackable
            TrackableResult result = state.getTrackableResult(tIdx);
            Trackable trackable = result.getTrackable();
          //  Log.d(LOGTAG, "num=	\"" + (String)(state.getNumTrackables()) + "\"");
            //state.getNumTrackables();
